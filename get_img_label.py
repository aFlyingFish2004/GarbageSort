import os
#该代码用于生成数据集的标签文件label.txt，文件内容格式如下：
...
with open(output_file, "w", encoding="utf-8") as f_out, open(error_log, "w", encoding="utf-8") as f_err:
    for root, dirs, files in os.walk("garbage"):
        dirs.sort()
        files.sort()
        try:
            for file in files:
